...
class DataRequest:
  ...
  def _start_session(self, auth) -> None:
    ...
    if res.status_code == 200:
      print(f'✅ Started session for: {self.data_request.name} {self.data_request.routing_key}')
    else:
      print(f'🚨 Failed to start session for: {self.data_request.name} {self.data_request.routing_key}')
